...
// Monorepo release-build fix (R1-T1). For an offline release bundle
// (`export:embed`), the React Native Gradle plugin relativizes `--entry-file`
...
//
// Scoped to export only (via argv) so the interactive dev server — which relies
// on the repo-root server root to serve hoisted packages over HTTP — is
// unchanged. The dev client (`expo start`) never carries an `export` arg.
const isExporting = process.argv.some((arg) => arg.includes('export'));
if (isExporting) {
